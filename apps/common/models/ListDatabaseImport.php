...
class ListDatabaseImport extends ListImportAbstract
{
    ...
    public function countResults()
    {
        $count = 0;
        if ($this->server_type == self::SERVER_TYPE_MYSQL) {
            $row   = $this->getDbConnection()->createCommand(sprintf('SELECT COUNT(*) AS counter FROM `%s`', $this->table_name))->queryRow();
            $count = $row['counter'];
        }
        return $count;
    }
    ...
    public function getResults($offset, $limit)
    {
        ...
        if ($this->server_type == self::SERVER_TYPE_MYSQL) {
            ...
            $results = $this->getDbConnection()->createCommand(sprintf('SELECT %s FROM `%s` ORDER BY 1 LIMIT %d OFFSET %d', $columns, $this->table_name, (int)$limit, (int)$offset))->queryAll();
        }
        return $results;
    }
}