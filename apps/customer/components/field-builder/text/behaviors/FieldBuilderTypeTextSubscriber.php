...
/**
 * FieldBuilderTypeTextSubscriber
 * 
...
class FieldBuilderTypeTextSubscriber extends CBehavior
{
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        // and save the subscriber model if no errors.
        if (empty($this->owner->errors)) {
            $subscriber->email = $request->getPost('EMAIL');
            if (!$subscriber->save()) {
                $this->owner->errors[] = array(
                    'show'      => true, 
                    'message'   => $subscriber->shortErrors->getAllAsString()
                );
            }
        }
    }
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        if ($event->sender->field->tag === 'EMAIL') {
            $checkMX = $checkPort = false;
            if (Yii::app()->options->get('system.common.dns_email_check', false)) {
                $checkMX    = CommonHelper::functionExists('checkdnsrr');
                $checkPort  = CommonHelper::functionExists('dns_get_record') && CommonHelper::functionExists('fsockopen');    
            }
            $validator = 'customer.components.field-builder.text.validators.FieldBuilderTypeTextEmailValidator';
            $rules->add(array('value', 'email', 'checkMX' => $checkMX, 'checkPort' => $checkPort));
            $rules->add(array('value', $validator, 'field' => $event->sender->field, 'subscriber' => $this->owner->getSubscriber() ));
        }
    }
