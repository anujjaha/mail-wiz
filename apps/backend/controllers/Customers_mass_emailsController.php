    ...
    public function actionIndex()
    {
        ...
        $this->setData(array(
            ...
            'pageBreadcrumbs'   => array(
                ...
                Yii::t('customers', 'Mass emails') => $this->createUrl('customers_mass_email/index'),
            )
        ));
        ...
        if ($request->isPostRequest && ($attributes = $request->getPost($model->modelName, array()))) {
            ...
            if (!$request->isAjaxRequest) {
                $rawContent = Yii::app()->params['POST'][$model->modelName]['message'];
                $parser = new EmailTemplateParser();
                $model->message = $parser->setContent($rawContent)->getContent();
                if ($model->validate()) {
